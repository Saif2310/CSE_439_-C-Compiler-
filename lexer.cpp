...
typedef vector<pair<string, int>> symbol_table_t;

symbol_table_t symbol_table;

int add_to_symbol_table(const std::string &token_name, int type)
{
    symbol_table.push_back(make_pair(token_name, type));
...
std::ostream &operator<<(std::ostream &out, const symbol_table_t &st)
{
    ...
    for (const auto &entry : st)
    {
        ...
        {
            out << "Token: " << entry.first << ", Type: " << entry.second;
            out << endl;
        }
    }
    return out;
...
{
    int symbol_table_index = add_to_symbol_table(token->lexeme, token->type);
...
int main()
{
    std::vector<std::string> file_chunks = readFileChunks("test4.txt");
    std::string buffer;
